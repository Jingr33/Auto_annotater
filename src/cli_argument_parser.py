...
class CLIArgumentParser:
    ...
    def _build_parser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--source", default=None,
                            help="Dataset root (images/ + labels/) or folder with images directly")
        parser.add_argument("--output", required=True,
                            help="Workspace folder for results (items/ + items.db)")

        parser.add_argument("--model", choices=[e.value for e in ModelType], default=None,
                            help="Model type for annotate step")
        parser.add_argument("--model-path", default=None,
                            help="Path to model weights (default: hardcoded per model)")

        parser.add_argument("--ssh-host", default=None,
                            help="SSH host for remote inference")
        ...
        parser.add_argument("--remote-work-dir", default="/tmp/medsam2",
                            help="Working directory on remote server")
        parser.add_argument("--remote-model-path", default=None,
                            help="Model path on remote server")
        parser.add_argument("--remote-python", default="python3",
                            help="Python executable on remote server (default: python3)")
