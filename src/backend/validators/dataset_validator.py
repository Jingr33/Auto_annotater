...
class DatasetValidator:
    def validate(self, source_path: str, model_type: ModelType | None) -> list[str]:
        ...
        if not os.path.isdir(images_dir):
            raise FileNotFoundError(
                f"Required images/ folder not found in dataset: {source_path}"
            )
        ...
        if model_type == ModelType.MEDSAM2:
            ...
            if not os.path.isdir(labels_dir):
                raise FileNotFoundError(
                    "MEDSAM2 dataset requires a labels/ folder with YOLO bounding boxes"
                )
    ...
    def _validate_label(self, labels_dir: str, image_file: str) -> None:
        ...
        if not os.path.isfile(label_file):
            raise FileNotFoundError(
                f"MEDSAM2 label missing for {image_file}: {label_file}"
            )
        ...
        for line_number, line in enumerate(lines, start=1):
            ...
            if len(parts) != 5:
                raise ValueError(
                    f"Invalid YOLO bbox in {label_file} line {line_number}: "
                    "expected 5 values"
                )
            ...
            if class_index < 0:
                raise ValueError(
                    f"Invalid class index in {label_file} line {line_number}"
                )
            if not all(0.0 <= value <= 1.0 for value in (x, y, width, height)):
                raise ValueError(
                    f"YOLO bbox values must be between 0 and 1 in "
                    f"{label_file} line {line_number}"
                )
            if width == 0.0 or height == 0.0:
                raise ValueError(
                    f"YOLO bbox width and height must be greater than zero in "
                    f"{label_file} line {line_number}"
                )
            if (
                x - width / 2.0 < 0.0
                ...
                raise ValueError(
                    f"YOLO bbox must remain within image bounds in "
                    f"{label_file} line {line_number}"
                )
