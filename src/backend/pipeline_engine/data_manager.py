...
class DataManager:
    ...
    def import_image(self, source_path: str, item_id: str | None = None) -> str:
        if item_id is None:
    ...
    def save_annotation(
        self, item_id: str, annotations: list[Annotation], label: AnnotationLabel = AnnotationLabel.YOLO
    ) -> str:
    ...
    def get_items(self, status: ImagePredictionStatus | None = None) -> list[dict]:
        if status:
            ...
            rows = self._conn.execute('SELECT id, status, created_at, updated_at FROM items ORDER BY created_at')
