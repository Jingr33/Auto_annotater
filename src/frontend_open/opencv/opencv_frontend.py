...
class OpenCVFrontend:
    ...
    def _draw_bars(self, canvas: np.ndarray) -> None:
        ...
        cv.putText(
            canvas, 'ACCEPT', (CFG.WIN_WIDTH - 80, CFG.WIN_HEIGHT // 2), FONT, FONT_SCL, TXT_CLR, THICK, cv.LINE_AA
        )
        cv.putText(canvas, 'BACK', (CFG.WIN_WIDTH // 2, 30), FONT, FONT_SCL, TXT_CLR, THICK, cv.LINE_AA)
        cv.putText(
            canvas, 'SKIP', (CFG.WIN_WIDTH // 2, CFG.WIN_HEIGHT - 30), FONT, FONT_SCL, TXT_CLR, THICK, cv.LINE_AA
        )
    ...
    def _draw_current_prediction(self, canvas: np.ndarray) -> None:
        ...
        canvas[CFG.BAR_HEIGHT : CFG.BAR_HEIGHT + img_h, CFG.BAR_WIDTH : CFG.BAR_WIDTH + img_w] = image
